...
class _SmgApi {
  ...
  get secrets() {
    ...
    const _secretClosure = ({ id: pSecretId } = {}) => {
      const path = '/secrets'
      const self = this

      // common poster
      const poster = ({ id, data, path }, ...params) => {
      ...
      const getter = ({ id, path }, ...params) => {
        return self.proxy(path + Utils.addParams(params.concat(id ? [{ secretId: id }] : []).concat(self.extraParams)), {
          contentType: 'application/json',
          method: "GET"
        })
      }
      const makeSecretPath = ({ id, extra = '' }) => path + `/${id}${extra}`
      const makeVersionPath = ({ id, versionId }) => makeSecretPath({
        id,
        extra: `/versions/${versionId}:access`
      })

      return {
        // iam permission on projectid - secretmanager.secrets.create
        create: ({ id = pSecretId } = {}, ...params) => poster({ id, data: self.replication, path }, ...params),
        addVersion: ({ id = pSecretId, data } = {}, ...params) => poster({
          data: { payload: { data: Utilities.base64Encode(JSON.stringify({ value: data })) } },
          path: makeSecretPath({ id, extra: ":addVersion" })
        }, ...params),
        get: ({ id = pSecretId, versionId = "latest" } = {}, ...params) => {
          const result = getter({
            path: makeVersionPath({ id, versionId })
          }, ...params)
          if (!result.error) {
            const unb = Utilities.newBlob(Utilities.base64Decode(result.data.payload.data)).getDataAsString()
            console.log(unb)
            result.data.value = JSON.parse(unb).value
          }
          return result
        }
      }
